...
import { Calendar, FileText, Users, Search, Send, Plus, ExternalLink, AlertCircle } from 'lucide-react';
import { useApp } from '../../contexts/AppContext';

export const PreparationPhase = ({ project }) => {
  const { advanceToNextPhase, updateProject } = useApp();
  
  const [method, setMethod] = useState('AOR');
              ...
              {Object.entries(keyDates).map(([key, value]) => (
                <div key={key}>
                  <label className="form-label">
                    ...
                    {['publication', 'responses'].includes(key) && ' *'}
                  </label>
                  <input
                    type="date"
                    value={value}
                    ...
                    required={['publication', 'responses'].includes(key)}
                  />
                </div>
              ))}
            </div>
